...
var _ = Describe("SSH proxy", func() {
	...
	Describe("authenticating with the cf realm with a bearer token", func() {
		BeforeEach(func() {
			clientConfig = &ssh.ClientConfig{
				User: "cf:app-guid/99",
		...
		Context("when authentication fails", func() {
			...
			It("logs the authentication failure", func() {
				_, err := ssh.Dial("tcp", address, clientConfig)
				Expect(err).To(MatchError(ContainSubstring("ssh: handshake failed")))
				Expect(runner).To(gbytes.Say("authentication-failed.*cf:app-guid/99"))
				Expect(fakeCC.ReceivedRequests()).To(HaveLen(1))
			})
		})
		...
		Context("when the app-guid does not exist in cc", func() {
			...
			It("attempts to acquire the app info from cc and fails the authentication", func() {
				_, err := ssh.Dial("tcp", address, clientConfig)
				Expect(err).To(MatchError(ContainSubstring("ssh: handshake failed")))
				Expect(fakeCC.ReceivedRequests()).To(HaveLen(1))
			})
		...
		Context("when the instance index does not exist", func() {
			BeforeEach(func() {
				clientConfig.User = "cf:app-guid/100"

				expectedGetActualLRPRequest.Index = 100
				actualLRPGroupResponse = &models.ActualLRPGroupResponse{
					Error:          models.ErrResourceNotFound,
					ActualLrpGroup: nil,
				}
