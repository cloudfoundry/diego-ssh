...
var _ = Describe("ForwardedTcpipChannelHandler", func() {
	var (
		sshd   *daemon.Daemon
		client *ssh.Client
		...
		testHandler *handlers.ForwardedTcpipChannelHandler
	...
	Context("when the forwarded-tcpip extra data fails to unmarshal", func() {
		It("rejects the open channel request", func() {
			_, _, err := client.OpenChannel("forwarded-tcpip", ssh.Marshal(struct{ Bogus int }{Bogus: 1234}))
	...
	Context("when an out of band request is sent across the channel", func() {
		...
		BeforeEach(func() {
			...
			forwardedTcpipMessage.ConnectedAddr = addr
			forwardedTcpipMessage.ConnectedPort = uint32(p)
		})

		It("rejects the requests", func() {
			channel, _, err := client.OpenChannel("forwarded-tcpip", ssh.Marshal(forwardedTcpipMessage))
			Expect(err).NotTo(HaveOccurred())
